...
export async function createBudget(data: {
  ...
  if (!data.title || !data.clientId || !data.createdById) {
    throw new Error(
      "Dados Invalidos: Titulo, CLientId e CreatedById são orbrigatorios"
    );
  }
  const prepareData = {
    ...data,
...
export async function findBudgetById(id: number) {
  ...
  if (!budget) {
    throw new Error("Orçamento nao encontrado");
  }
  return budget;
}
...
export async function deleteBudget(id: number) {
  ...
  if (!existingBudget) {
    {
      throw new Error("Orcamento nao encontrado para exclusao");
    }
  }
